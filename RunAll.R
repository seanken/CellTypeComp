...
tic();res<-test_deseq2(meta,condition,celltype,individual);nams=names(res);pvals=as.numeric(res);dat=data.frame(nams);dat["pval"]=pvals;colnames(dat)[1]="CellType";dat["method"]="DESeq2";out=toc();dat["time"]=out$toc-out$tic;ret=rbind(ret,dat);times<-c(times,out$tic-out$toc);
print(9);
